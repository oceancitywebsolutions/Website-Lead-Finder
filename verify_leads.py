...
search (via the Brave Search API) and adds a `possible_website_found`
column so a human can quickly confirm or discard borderline leads before
pitching them - it's a hint, not an automatic filter, since name-to-domain
matching isn't foolproof.
"""
...
def find_possible_website(session, api_key, name, town):
...
def main():
    ...
    parser.add_argument("--delay", type=float, default=1.0, help="Seconds to wait between search queries (Brave's free tier is rate-limited to 1/sec).")
    ...
    if not api_key:
        print(
            "Error: BRAVE_API_KEY is not set. See README for how to get a free "
